...
function gameOver() {
  ...
  // Update buttons
  if (stopQuestBtn) stopQuestBtn.classList.add('hidden');
  if (startAgainBtn) startAgainBtn.classList.remove('hidden');

  // Update UI to Game Over state
  const arenaSpriteEl = document.getElementById('arena-sprite');
  if (arenaSpriteEl) {
    arenaSpriteEl.innerHTML = "💀";
    arenaSpriteEl.classList.remove("animate-pulse", "hover:scale-110");
    arenaSpriteEl.classList.add("opacity-50");
  }
  ...
  logMessage(`GAME OVER. Start again to redeem yourself.`);
}
